...
function updateBoss(dt, t){
  ...
  if(BOSS.projs){
    for(const p of BOSS.projs){
      ...
      // 지면 충돌 체크
      const gy = cityFloorHeight(p.mesh.position.x, p.mesh.position.z);
      if(p.mesh.position.y < gy + 0.25){
        p.on = false; p.mesh.visible = false;
        // 보스의 슬러지는 도시 안에서도 착탄 지점에 남는다.
  ...
  if(BOSS.puddles){
    for(const pd of BOSS.puddles){
      ...
      if(dp < 4.2 && P.onGround){
        P.speed *= 0.82; // 실제 이동에 사용하는 속도를 둔화
        if(STATE.inv <= 0){
  ...
  /* 경계 안쪽에 서 있어도 보스의 접촉 범위에 들어오면 피해 */
