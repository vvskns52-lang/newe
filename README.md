...
## 수업 관련
사당 10곳의 학습 내용과 확인 문제는 `js/config.js`, 시뮬레이션 구현은 `js/puzzles.js`에서 확인할 수 있습니다.

## 수정 후 검증
Node.js가 설치되어 있으면 `node tests/regression.cjs`로 사당 종료 시 예약 작업 취소, 체력 확장, 힌트 재열람, 무기 승급 안내, 입력 초기화를 검사할 수 있습니다.
이 테스트는 게임 로직 검사이며, 실제 3D 화면과 터치 조작은 브라우저에서 별도로 확인해야 합니다.
